...
impl Drop for Droppable {
    ...
    // implementation of the Drop trait needs only to deallocate resources
...
fn custom_drop_example() {
    ...
    // Won't compile as Rust does not allow explicit calls to the drop method:
    // _droppable.drop();
    // If you want to early drop the value, call the `drop()` function
    // drop(_droppable);
...
fn iterate_taking_ownership_of_elements_example() {
    ...
    // In the standard library, there is often an `iter()` method present that
    // iterates over references to values, and convenience implementations
    // `IntoIterator` on references to a type. (Remember reference is part of the type!)
    for borrowed in &v3 {
        println!("{} is just borrowed", borrowed.name);
    }
