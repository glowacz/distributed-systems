...
fn closure_environment() {
    ...
    println!("closure_environment: {filtered:?}, the {allowed_numbers:?} are still there");
}
...
fn environment_capture_rules() {
    ...
    // `drop()` requires `T` so the `movable` variable has to be captured
    ...
    let consume = || {
        ...
        // Dispose the value:
        drop(movable);
        // `drop()` has a very interesting implementation... it is empty.
...
// Closures are always anonymous types and are typically used through a trait `Fn`
// with a special syntax: `Fn(Args) -> Ret`.
fn returns_a_closure(arg: String) -> impl Fn() -> usize {
    move || {
        println!("closure called: {arg}");
        arg.len()
    }
}
fn takes_a_closure(cb: impl Fn() -> usize) {
    let _size = cb();
}
fn juggles_closures() {
    let a: String = "hello".into();
    let closure = returns_a_closure(a);
    takes_a_closure(closure);
}

// However, non-locally living closures need trait objects and heap allocation.
// This way, closures can be stored in structs for later use:
fn closure_trait() -> Box<dyn Fn(u32) -> bool> {
    let even: Box<dyn Fn(u32) -> bool> = Box::new(|x: u32| x.is_multiple_of(2));
