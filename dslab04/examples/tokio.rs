...
// As of 2025, by default it does not use the operating system's asynchronous IO.
// Instead, under the hood it wraps blocking calls in `tokio::task::spawn_blocking()`.
// However, there is an experimental io_uring-based implementation.
#[tokio::main]
...
async fn tokio_sync() {
    ...
    // the same as in a standard multithreaded code:
...
// The runtime may be also specified as arguments to the macro.
// An important feature of tokio is possible virtualization of time:
// we don't need to actually sleep for a second to execute a system with a delay.
#[tokio::main(flavor = "current_thread", start_paused = true)]
async fn tokio_virtual_time() {
    // The true system time: don't use this during our labs based on `tokio`!
    let start_instant = std::time::Instant::now();
    // An opaque "current time" representation by `tokio`.
    // The time advances only when it is needed to progress with the tokio runtime.
    let tokio_start_instant = tokio::time::Instant::now();

    // This will advance the virtual time by one hour...
    tokio::time::sleep(Duration::from_secs(3600)).await;

    let elapsed = start_instant.elapsed();
    let tokio_vtime_elapsed = tokio_start_instant.elapsed();
    println!("Virtual time elapsed: {tokio_vtime_elapsed:?}, but wall clock only {elapsed:?}.");
}
