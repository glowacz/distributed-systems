...
/// They can be then easily converted to HTML (by calling `cargo doc`), just
...
fn simple_function() {
    ...
    // We put no semicolon after the last expression of the function, so this value: unit
    // (the result of the macro call) is returned from the function.
    // Note: you should typically put a semicolon there when returning the unit value.
}
...
fn mutability() {
    ...
    // Won't compile:
    // v1.truncate(1);
    println!("Immutable vector (heap-allocated) with two i64 values: {v1:?}");

    let mut v2: Vec<i64> = vec![0, 1];
    v2.truncate(1);
    println!("Mutable vector (heap-allocated) with two i64 values: {v2:?}");
