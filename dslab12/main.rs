...
use domain::{Action, Edit, EditRequest, EditorClient, Operation, ReliableBroadcast};
...
async fn main() {
    ...
    let processes = [
        system
            .register_module(|_| {
                Process::new(0, Box::new(broadcast.clone()), Box::new(client0.clone()))
            })
            .await,
        system
            .register_module(|_| {
                Process::new(1, Box::new(broadcast.clone()), Box::new(client1.clone()))
            })
            .await,
    ];
...
/// Exemplary client implementation.
///
...
/// Exemplary broadcast implementation.
///
...
impl<const N: usize> SimpleBroadcast<N> {
    pub(crate) async fn new(system: &mut System) -> ModuleRef<Self> {
        let self_ref = system.register_module(|_| Self { processes: None }).await;
        self_ref
    }
}
